...
SOURCE_URL = 'https://oslmusic.org/'
SOURCE = "Orchestra of St. Luke's"
...
def event_to_record(event):
    ...
    # The API includes old online, test, and incomplete entries. Records without
    # a physical venue and city cannot satisfy the production data contract.
...
def scrape_events(session=None):
    ...
    while True:
        ...
        if not isinstance(events, list):
            raise ValueError('Events API response did not contain an events list')
    ...
    if skipped_count:
        log_message(
            'Skipped calendar entries missing required event fields',
...
class OslmusicOrgCrawler(BaseCrawler):
